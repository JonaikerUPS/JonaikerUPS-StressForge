...
export class DefaultAdapter implements ToolAdapter {
  ...
  parse(rawOutput: string): UnifiedMetrics | null {
    ...
    for (const re of throughputRegexes) {
      ...
      if (match) { throughput = parseFloat(match[1]); break; }
    }

    let avg = 0;
    for (const re of latencyRegexes) {
      const match = rawOutput.match(re);
      if (match) { avg = parseFloat(match[1]); break; }
    }

    let hasError = false;
    for (const re of errorRegexes) {
      if (rawOutput.match(re)) { hasError = true; break; }
    }

    if (throughput === 0 && avg === 0 && !hasError) return null;
